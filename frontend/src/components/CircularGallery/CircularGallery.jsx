...
function createTextTexture(gl, text, font = 'bold 30px monospace', color = 'black') {
  ...
  canvas.width = textWidth + 40; // Add padding to avoid clipping
...
class App {
  ...
  update() {
    ...
    // Call the callback to update the active index based on current scroll position
    if (this.medias && this.medias[0] && this.onChangeActiveIndex && this.originalLength) {
      const width = this.medias[0].width;
      const rawIndex = Math.round(Math.abs(this.scroll.current) / width);
      const normalizedIndex = (rawIndex % this.originalLength + this.originalLength) % this.originalLength;
      this.onChangeActiveIndex(normalizedIndex);
    }
